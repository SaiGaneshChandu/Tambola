  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-slate-900 p-4">
      <div className="bg-slate-800 p-8 rounded-3xl shadow-2xl w-full max-w-md border border-slate-700">
        ...
        <input className="w-full p-3 mb-4 bg-slate-900 rounded-xl text-white border border-slate-600 focus:border-yellow-400 outline-none" 
               onChange={(e) => setFormData({...formData, name: e.target.value})} />

        <label className="text-slate-400 text-xs font-bold uppercase">Max Players (1-15)</label>
        <select className="w-full p-3 mb-4 bg-slate-900 rounded-xl text-white border border-slate-600"
        ...
        <input type="password" maxLength="4" className="w-full p-3 mb-8 bg-slate-900 rounded-xl text-white border border-slate-600"
               onChange={(e) => setFormData({...formData, password: e.target.value})} />

        <button onClick={() => onJoin(formData)} className="w-full bg-yellow-500 hover:bg-yellow-400 text-black font-black py-4 rounded-2xl transition-all active:scale-95">
...
function App() {
  ...
  const handleJoin = (data) => {
    ...
    // WebSocket connect
    socket.current = new WebSocket(`ws://127.0.0.1:8000/ws/game/${rId}/`);
    socket.current.onmessage = (e) => {
      const msg = json.loads(e.data);
      if (msg.type === 'NEW_NUMBER') {
        setCurrentNumber(msg.number);
        setCalledNumbers(prev => [...prev, msg.number]);
      }
    };
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6 bg-slate-800 p-4 rounded-2xl border-b-4 border-yellow-500">
          <div>
            ...
            <p className="text-[10px] text-slate-400">Invite: {window.location.origin}/join/{roomId}</p>
          </div>
          <button onClick={() => socket.current.send(JSON.stringify({action: 'START_GAME'}))}
                  className="bg-green-600 px-6 py-2 rounded-full font-bold hover:bg-green-500">START</button>
        </div>
        ...
        <div className="grid grid-cols-10 gap-1 bg-slate-800 p-4 rounded-xl">
          {[...Array(90)].map((_, i) => (
            <div key={i+1} className={`h-8 flex items-center justify-center text-[10px] rounded ${calledNumbers.includes(i+1) ? 'bg-yellow-500 text-black font-bold' : 'bg-slate-700 text-slate-500'}`}>
              {i+1}
            </div>
          ))}
        </div>
