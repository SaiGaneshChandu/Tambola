            ...
            {!joinData.isJoined ? (
                ...
                <div className="w-full max-w-2xl bg-slate-900 p-8 rounded-3xl border-b-4 border-yellow-500 shadow-2xl">
                    ...
                    <div className="bg-black p-4 rounded-xl border border-slate-800 text-center text-xs">
                        ...
                        <p className="text-yellow-500 font-mono mb-4 break-all">{window.location.href}</p>
                        <button onClick={() => {navigator.clipboard.writeText(window.location.href); alert("Link Copied!")}} 
                            className="bg-slate-800 px-4 py-1 rounded border border-slate-700">Copy Link</button>
                    </div>
                </div>
            )}
        </div>
